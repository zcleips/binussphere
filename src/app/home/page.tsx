...
function PostCard({
  ...
  <div className="w-12 h-12 rounded-full bg-yellow-100 flex items-center justify-center font-bold text-yellow-600 text-lg cursor-pointer hover:opacity-90 transition">
    {post.name[0].toUpperCase()}
        ...
        <div className="flex-1">
          <div className="flex justify-between">
            <div>
              <Link href={`/profile/${post.name}`}>
  <h3 className="font-bold text-slate-900 dark:text-slate-100 hover:underline cursor-pointer">
    {post.name}
  </h3>
</Link>
            ...
            {post.isMine ? (
              ...
              <div className="relative">
                ...
                {showReportMenu && (
                  <div className="absolute right-0 mt-2 w-44 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-lg overflow-hidden z-20">
        ...
        <div className="flex gap-4 mt-1 ml-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
          ...
          {comment.isMine ? (
            ...
            <div className="relative">
              ...
              {showReportMenu && (
                <div className="absolute left-0 mt-2 w-40 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-lg overflow-hidden z-20">
                  {["Abusive", "Hate speech", "Nudity", "Others"].map(
                    (reason) => (
                      <button
                        key={reason}
                        onClick={() => submitReport(reason)}
                        className="block w-full text-left px-4 py-2 text-xs text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700"
                      >
                        {reason}
                      </button>
                    )
